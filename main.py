...
print(f"training data keys: {train_file.files}")

# TODO identify bug crash here. Memory crash at least, attempting to mitigate with 2 batch size.
...
"""
Memory Bug:
    - Setting pin_memory to false in hopes that CPU memory is bottleneck
    - Lowering workers from 8 to 0
    Confirmed it is a CPU memory issue on HTOP
    Possible solutions
        - Preprocess data transformations
        - Check for data leaks
        - Increase swap memory
    - Adjusting swap size to 16GB, not finished yet
"""
train_dataloader = DataLoader(train_dataset, batch_size=cfg.BATCH_SIZE, shuffle=True, num_workers=0, pin_memory=False, collate_fn=custom_collate_fn, drop_last=True)
val_dataloader = DataLoader(val_dataset, batch_size=cfg.BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=False, collate_fn=custom_collate_fn, drop_last=True)
